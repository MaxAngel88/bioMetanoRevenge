...
import pojo.AgreementUpdateStatusPojo
...
object AgreementFlow {
    ...
    class IssuerAgreement(val agreementProperty: AgreementPojo) : FlowLogic<AgreementState>() {
        ...
        override fun call(): AgreementState {
            ...
            val agreementState = AgreementState(
                    ...
                    agreementProperty.plantID,
                    agreementProperty.plantAddress,
                    agreementProperty.plantBusinessName,
                    agreementProperty.owner,
                    agreementProperty.counterpart,
                    agreementProperty.remiCode,
                    agreementProperty.averageDailyCapacity,
                    agreementProperty.maxDailyCapacity,
                    agreementProperty.energyEstimation,
    ...
    class UpdaterAgreement(val agreementUpdateProperty: AgreementUpdatePojo) : FlowLogic<AgreementState>() {
        ...
        override fun call(): AgreementState {
            ...
            val newAgreementState = AgreementState(
                    ...
                    oldAgreementState.plantID,
                    oldAgreementState.plantAddress,
                    oldAgreementState.plantBusinessName,
                    oldAgreementState.owner,
                    oldAgreementState.counterpart,
                    oldAgreementState.remiCode,
                    oldAgreementState.averageDailyCapacity,
                    oldAgreementState.maxDailyCapacity,
                    oldAgreementState.energyEstimation,
                    ...
                    oldAgreementState.invoicingCode,
                    ...
                    oldAgreementState.agreementStatus,
    ...
    /***
     *
     * Update Agreement Status Flow -----------------------------------------------------------------------------------
     *
     * */
    @InitiatingFlow
    @StartableByRPC
    class UpdaterAgreementStatus(val agreementUpdateStatusProperty: AgreementUpdateStatusPojo) : FlowLogic<AgreementState>() {
        /**
         * The progress tracker checkpoints each stage of the flow and outputs the specified messages when each
         * checkpoint is reached in the code. See the 'progressTracker.currentStep' expressions within the call() function.
         */
        companion object {
            object GENERATING_TRANSACTION : ProgressTracker.Step("Generating transaction based on update status Agreement.")
            object VERIFYIGN_TRANSACTION : ProgressTracker.Step("Verifying contract constraints.")
            object SIGNING_TRANSACTION : ProgressTracker.Step("Signing transaction with our private key.")
            object GATHERING_SIGS : ProgressTracker.Step("Gathering the other Party signature.") {
                override fun childProgressTracker() = CollectSignaturesFlow.tracker()
        ...
        override fun call(): AgreementState {
            ...
            // setting the criteria for retrive UNCONSUMED state AND filter it for agreementID
            var agreementIDCriteria: QueryCriteria = QueryCriteria.VaultCustomQueryCriteria(expression = builder { AgreementSchemaV1.PersistentAgreement::agreementID.equal(agreementUpdateStatusProperty.agreementID) }, status = Vault.StateStatus.UNCONSUMED, contractStateTypes = setOf(AgreementState::class.java))
            ...
            if (oldAgreementStateList.size > 1 || oldAgreementStateList.isEmpty()) throw FlowException("No agreement state with agreementID: ${agreementUpdateStatusProperty.agreementID} found.")
            ...
            val newAgreementState = AgreementState(
                    ...
                    oldAgreementState.agreementNote,
                    oldAgreementState.invoicingCode,
                    oldAgreementState.energy,
                    oldAgreementState.dcq,
                    oldAgreementState.price,
                    oldAgreementState.validFrom,
                    oldAgreementState.validTo,
                    agreementUpdateStatusProperty.agreementStatus,
                    oldAgreementState.agreementDate,
                    Instant.now(),
                    UniqueIdentifier(id = UUID.randomUUID())
            )

            val txCommand = Command(AgreementContract.Commands.UpdateStatus(), newAgreementState.participants.map { it.owningKey })
            val txBuilder = TransactionBuilder(notary)
                    .addInputState(oldAgreementStateRef)
                    .addOutputState(newAgreementState, AgreementContract.ID)
                    .addCommand(txCommand)
    ...
    @InitiatedBy(UpdaterAgreementStatus::class)
    class UpdateAcceptorAgreementStatus(val otherPartySession: FlowSession) : FlowLogic<SignedTransaction>() {
        @Suspendable
        override fun call(): SignedTransaction {
            val signTransactionFlow = object : SignTransactionFlow(otherPartySession) {
                override fun checkTransaction(stx: SignedTransaction) = requireThat {

...
import pojo.EnrollUpdateOCRPojo
...
object EnrollFlow {
    ...
    class IssuerEnroll(val enrollProperty: EnrollPojo) : FlowLogic<EnrollState>() {
        ...
        override fun call(): EnrollState {
            ...
            val enrollState = EnrollState(
                    ...
                    enrollProperty.docDeadLineAuto,
                    enrollProperty.docDeadLineTech,
    ...
    class UpdaterEnroll(val enrollUpdateProperty: EnrollUpdatePojo) : FlowLogic<EnrollState>() {
        ...
        override fun call(): EnrollState {
            ...
            val newEnrollState = EnrollState(
                    ...
                    oldEnrollState.docDeadLineAuto,
                    oldEnrollState.docDeadLineTech,
    ...
    /***
     *
     * Update OCR Enroll Flow -----------------------------------------------------------------------------------
     *
     * */
    @InitiatingFlow
    @StartableByRPC
    class UpdaterOCREnroll(val enrollUpdateOCRProperty: EnrollUpdateOCRPojo) : FlowLogic<EnrollState>() {
        /**
         * The progress tracker checkpoints each stage of the flow and outputs the specified messages when each
         * checkpoint is reached in the code. See the 'progressTracker.currentStep' expressions within the call() function.
         */
        companion object {
            object GENERATING_TRANSACTION : ProgressTracker.Step("Generating transaction based on update OCR Enroll.")
            object VERIFYIGN_TRANSACTION : ProgressTracker.Step("Verifying contract constraints.")
            object SIGNING_TRANSACTION : ProgressTracker.Step("Signing transaction with our private key.")
            object GATHERING_SIGS : ProgressTracker.Step("Gathering the other Party signature.") {
                override fun childProgressTracker() = CollectSignaturesFlow.tracker()
        ...
        override fun call(): EnrollState {
            ...
            // setting the criteria for retrive UNCONSUMED state AND filter it for uuid
            var uuidCriteria: QueryCriteria = QueryCriteria.VaultCustomQueryCriteria(expression = builder { EnrollSchemaV1.PersistentEnroll::uuid.equal(enrollUpdateOCRProperty.uuid) }, status = Vault.StateStatus.UNCONSUMED, contractStateTypes = setOf(EnrollState::class.java))
            ...
            if (oldEnrollStateList.size > 1 || oldEnrollStateList.isEmpty()) throw FlowException("No enroll state with uuid: ${enrollUpdateOCRProperty.uuid} found.")

            val oldEnrollStateRef = oldEnrollStateList[0]
            val oldEnrollState = oldEnrollStateRef.state.data

            // Generate an unsigned transaction.
            val newOCREnrollState = EnrollState(
                    GSEParty,
                    owner,
                    oldEnrollState.enrollType,
                    enrollUpdateOCRProperty.subjectFirstName,
                    enrollUpdateOCRProperty.subjectLastName,
                    enrollUpdateOCRProperty.subjectAddress,
                    enrollUpdateOCRProperty.subjectBusiness,
                    oldEnrollState.qualificationCode,
                    enrollUpdateOCRProperty.businessName,
                    enrollUpdateOCRProperty.PIVA,
                    oldEnrollState.birthPlace,
                    enrollUpdateOCRProperty.remiCode,
                    oldEnrollState.remiAddress,
                    enrollUpdateOCRProperty.idPlant,
                    enrollUpdateOCRProperty.plantAddress,
                    oldEnrollState.username,
                    oldEnrollState.role,
                    oldEnrollState.partner,
                    enrollUpdateOCRProperty.docRefAutodichiarazione,
                    enrollUpdateOCRProperty.docRefAttestazioniTecniche,
                    enrollUpdateOCRProperty.docDeadLineAuto,
                    enrollUpdateOCRProperty.docDeadLineTech,
                    oldEnrollState.enrollStatus,
                    oldEnrollState.enrollDate,
                    oldEnrollState.bioGasAmount,
                    oldEnrollState.gasAmount,
                    oldEnrollState.uuid,
                    Instant.now(),
                    UniqueIdentifier(id = UUID.randomUUID())
            )

            val txCommand = Command(EnrollContract.Commands.UpdateOCR(), newOCREnrollState.participants.map { it.owningKey })
            val txBuilder = TransactionBuilder(notary)
                    .addInputState(oldEnrollStateRef)
                    .addOutputState(newOCREnrollState, EnrollContract.ID)
                    .addCommand(txCommand)
            ...
            return newOCREnrollState
        }
    }

    @InitiatedBy(UpdaterOCREnroll::class)
    class UpdateOCRAcceptorEnroll(val otherPartySession: FlowSession) : FlowLogic<SignedTransaction>() {
        @Suspendable
        override fun call(): SignedTransaction {
            val signTransactionFlow = object : SignTransactionFlow(otherPartySession) {
                override fun checkTransaction(stx: SignedTransaction) = requireThat {

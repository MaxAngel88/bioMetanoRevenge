...
import pojo.BatchUpdateCheckPojo
...
object BatchFlow {
    ...
    class UpdaterBatch(val batchUpdateProperty: BatchUpdatePojo) : FlowLogic<BatchState>() {
        ...
        override fun call(): BatchState {
            ...
            val newBatchState = BatchState(
                    ...
                    oldBatchState.snamCheck,
                    oldBatchState.financialCheck,
    ...
    class UpdaterBatchAuction(val batchUpdateAuctionProperty: BatchUpdateAuctionPojo) : FlowLogic<BatchState>() {
        ...
        override fun call(): BatchState {
            ...
            if (myLegalIdentityNameOrg != "GSE") {
                throw FlowException("Node $myLegalIdentity cannot start the update-batch-auction flow. This flow can only be started from the GSE node")
            }
    ...
    /***
     *
     * Update Batch Check Flow -----------------------------------------------------------------------------------
     *
     * */
    @InitiatingFlow
    @StartableByRPC
    class UpdaterBatchCheck(val batchUpdateCheckProperty: BatchUpdateCheckPojo) : FlowLogic<BatchState>() {
        /**
         * The progress tracker checkpoints each stage of the flow and outputs the specified messages when each
         * checkpoint is reached in the code. See the 'progressTracker.currentStep' expressions within the call() function.
         */
        companion object {
            object GENERATING_TRANSACTION : ProgressTracker.Step("Generating transaction based on update Batch Check.")
            object VERIFYIGN_TRANSACTION : ProgressTracker.Step("Verifying contract constraints.")
            object SIGNING_TRANSACTION : ProgressTracker.Step("Signing transaction with our private key.")
            object GATHERING_SIGS : ProgressTracker.Step("Gathering the other Party signature.") {
                override fun childProgressTracker() = CollectSignaturesFlow.tracker()
        ...
        override fun call(): BatchState {
            ...
            if (myLegalIdentityNameOrg != "GSE") {
                throw FlowException("Node $myLegalIdentity cannot start the update-batch-check flow. This flow can only be started from the GSE node")
            }
            ...
            // setting the criteria for retrive UNCONSUMED state AND filter it for batchID
            var batchIDCriteria: QueryCriteria = QueryCriteria.VaultCustomQueryCriteria(expression = builder { BatchSchemaV1.PersistentBatch::batchID.equal(batchUpdateCheckProperty.batchID) }, status = Vault.StateStatus.UNCONSUMED, contractStateTypes = setOf(BatchState::class.java))
            ...
            if (oldBatchStateList.size > 1 || oldBatchStateList.isEmpty()) throw FlowException("No batch state with batchID: ${batchUpdateCheckProperty.batchID} found.")
            ...
            val newBatchState = BatchState(
                    ...
                    oldBatchState.supportField,
                    oldBatchState.auctionStatus,
                    batchUpdateCheckProperty.snamCheck,
                    batchUpdateCheckProperty.financialCheck,
                    oldBatchState.batchDate,
                    Instant.now(),
                    UniqueIdentifier(id = UUID.randomUUID())
            )

            val txCommand = Command(BatchContract.Commands.UpdateCheck(), newBatchState.participants.map { it.owningKey })
            val txBuilder = TransactionBuilder(notary)
                    .addInputState(oldBatchStateRef)
                    .addOutputState(newBatchState, BatchContract.ID)
                    .addCommand(txCommand)
    ...
    @InitiatedBy(UpdaterBatchCheck::class)
    class UpdateCheckAcceptorBatch(val otherPartySession: FlowSession) : FlowLogic<SignedTransaction>() {
        @Suspendable
        override fun call(): SignedTransaction {
            val signTransactionFlow = object : SignTransactionFlow(otherPartySession) {
                override fun checkTransaction(stx: SignedTransaction) = requireThat {
                    ...
                    /* "other rule batch" using (output is new rule) */
                    "batchID cannot be empty" using (batchState.batchID.isNotEmpty())
                    "snamCheck cannot be empty" using (batchState.snamCheck.isNotEmpty())
                    "financialCheck cannot be empty" using (batchState.financialCheck.isNotEmpty())
                }
            }
            val txId = subFlow(signTransactionFlow).id
